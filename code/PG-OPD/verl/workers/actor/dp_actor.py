...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _dump_ve_opd_token_traces(
        ...
        *,
        ...
        """Dump diagnostic-only per-token native/privileged teacher comparisons."""
        dump_root = self_distillation_cfg.get("token_trace_dump_dir", None)
        if not dump_root or not records:
            return
        global_step = meta_info.get("global_steps")
        if global_step is None:
            return
        every = int(self_distillation_cfg.get("token_trace_every_n_steps", 1))
        ...
        save_dir = (
            normalized_root
            if os.path.basename(normalized_root) == experiment_name
            else os.path.join(normalized_root, experiment_name)
        )
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f"step-{int(global_step):06d}.jsonl")
    ...
    def _forward_micro_batch(
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if self.use_remove_padding:
                ...
                # Qwen3.5 exposes logits_to_keep. With response_start_idx available,
                # compute logits only for valid response positions. Response padding can be
                # absent from attention_mask, so keep it out of the packed gather and fill
                # its returned log-prob/top-k slots with zeros, matching the response mask.
                response_only_logits = response_start_idx is not None and not self.use_ulysses_sp
                if response_only_logits:
                    ...
                    flat_response_positions = (
                        torch.arange(batch_size, device=indices.device, dtype=indices.dtype).unsqueeze(1) * seqlen
                        + response_positions.to(device=indices.device, dtype=indices.dtype)
                    ).reshape(-1)
                    packed_response_positions = torch.searchsorted(indices, flat_response_positions)
                    in_bounds = packed_response_positions < indices.numel()
                    valid_mask = torch.zeros_like(in_bounds)
                    if bool(in_bounds.any()):
                        valid_packed = packed_response_positions[in_bounds]
                        valid_flat = flat_response_positions[in_bounds]
                        valid_mask[in_bounds] = indices.index_select(0, valid_packed).eq(valid_flat)
                    if not bool(valid_mask.any()):
                        raise RuntimeError("No valid response positions found in packed attention mask")
                    valid_packed = packed_response_positions[valid_mask]
                    response_labels = input_ids_rmpad_rolled.index_select(0, valid_packed).reshape(-1)
                    response_extra_args = {}
                    if self.use_fused_kernels:
                        response_extra_args["temperature"] = temperature
                        response_extra_args["return_dict"] = True
                    response_extra_args["logits_to_keep"] = valid_packed
                    ...
                    if self.use_fused_kernels:
                        raise RuntimeError("Response-only logits path requires unfused logits")
                    response_logits = response_output.logits
                    if response_logits.dim() == 3:
                        response_logits = response_logits.squeeze(0)
                    response_logits = response_logits.reshape(-1, response_logits.shape[-1])
                    expected_rows = valid_packed.numel()
                    packed_rows = input_ids_rmpad.shape[-1]
                    # Some Qwen/transformers wrappers ignore tensor-valued logits_to_keep
                    # and return all packed positions. Select the requested response rows
                    # in that case; otherwise keep the already-sliced response logits.
                    if response_logits.shape[0] == packed_rows and packed_rows != expected_rows:
                        response_logits = response_logits.index_select(0, valid_packed)
                    if response_logits.shape[0] != expected_rows:
                        raise RuntimeError(
                            f"Response logits/labels mismatch: logits={response_logits.shape[0]}, "
                            f"labels={expected_rows}, packed={packed_rows}"
                        )
                    response_logits.div_(temperature)
                    response_log_probs = logprobs_from_logits(response_logits, response_labels)
                    total_response_tokens = batch_size * response_length
                    response_log_probs_flat = response_log_probs.new_zeros(total_response_tokens)
                    response_log_probs_flat[valid_mask] = response_log_probs
                    response_outputs = {"log_probs": response_log_probs_flat.view(batch_size, response_length)}
                    if calculate_entropy:
                        response_entropy = self.compute_entropy_from_logits(response_logits)
                        response_entropy_flat = response_entropy.new_zeros(total_response_tokens)
                        response_entropy_flat[valid_mask] = response_entropy
                        response_outputs["entropys"] = response_entropy_flat.view(batch_size, response_length)
                    if calculate_sum_pi_squared:
                        response_sum_pi_squared = self.calculate_sum_pi_squared_from_logits(response_logits)
                        response_sum_flat = response_sum_pi_squared.new_zeros(total_response_tokens)
                        response_sum_flat[valid_mask] = response_sum_pi_squared
                        response_outputs["sum_pi_squared"] = response_sum_flat.view(batch_size, response_length)
                    if compute_all_logps:
                        response_all_logps = torch.log_softmax(response_logits, dim=-1)
                        response_all_flat = response_all_logps.new_zeros(
                            total_response_tokens, response_all_logps.shape[-1]
                        )
                        response_all_flat[valid_mask] = response_all_logps
                        response_outputs["all_logps"] = response_all_flat.view(
                            batch_size, response_length, response_all_logps.shape[-1]
                        )
                    if use_topk:
                        if topk_indices is None:
                            topk = min(distill_topk, response_logits.size(-1))
                            response_topk_logits, response_topk_indices = torch.topk(response_logits, topk, dim=-1)
                        else:
                            response_topk_indices = topk_indices.reshape(-1, topk_indices.size(-1)).to(
                                device=response_logits.device, dtype=torch.long
                            )[valid_mask]
                            response_topk_logits = torch.gather(response_logits, dim=-1, index=response_topk_indices)
                        response_logsumexp = torch.logsumexp(response_logits, dim=-1, keepdim=True)
                        response_topk_logps = response_topk_logits - response_logsumexp
                        topk_flat = response_topk_logps.new_zeros(total_response_tokens, response_topk_logps.shape[-1])
                        topk_flat[valid_mask] = response_topk_logps
                        response_outputs["topk_logps"] = topk_flat.view(
                            batch_size, response_length, response_topk_logps.shape[-1]
                        )
                        if return_topk_indices:
                            indices_flat = response_topk_indices.new_zeros(
                                total_response_tokens, response_topk_indices.shape[-1]
                            )
                            indices_flat[valid_mask] = response_topk_indices
                            response_outputs["topk_indices"] = indices_flat.view(
                                batch_size, response_length, response_topk_indices.shape[-1]
                            )
                    return response_outputs

                # Preserve the original model call for the fallback path.
                extra_args = {}
                if self.use_fused_kernels:
    ...
    def update_policy(self, data: DataProto):
        ...
        if self_distillation_enabled:
            ...
            if self_distillation_cfg.get("teacher_negative_image_key", None) is not None:
                self_distillation_required_keys.update(
                    {
                        "teacher_negative_input_ids",
                        "teacher_negative_attention_mask",
                        "teacher_negative_position_ids",
                        "teacher_negative_response_start_idx",
                    }
                )
                assert self_distillation_required_keys.issubset(set(data.batch.keys())), (
                    f"Missing negative teacher keys: {self_distillation_required_keys - set(data.batch.keys())}"
                )
        ...
        token_trace_enabled = bool(
            self_distillation_enabled
            and self_distillation_cfg.get("token_trace_dump_dir", None)
            and int(self_distillation_cfg.get("token_trace_samples_per_role", 0)) > 0
        )
        token_trace_selected = set()
        if token_trace_enabled:
            batch_size = data.batch.batch_size[0]
            roles = list(data.non_tensor_batch.get("ve_data_role", ["unknown"] * batch_size))
            rng = random.Random(
                int(self_distillation_cfg.get("token_trace_seed", 20260911))
                + int(data.meta_info.get("global_steps", 0))
            )
            per_role = int(self_distillation_cfg.get("token_trace_samples_per_role", 2))
            for role_name in sorted({str(role) for role in roles}):
                candidates = [idx for idx, role in enumerate(roles) if str(role) == role_name]
                rng.shuffle(candidates)
                token_trace_selected.update(candidates[:per_role])
            data.non_tensor_batch["ve_diag_index"] = np.arange(batch_size, dtype=np.int64)
            data.non_tensor_batch["ve_diag_selected"] = np.asarray(
                [idx in token_trace_selected for idx in range(batch_size)], dtype=np.bool_
            )
        ...
        has_negative_teacher_multi_modal_inputs = self._has_non_empty_multi_modal_inputs(
            data.non_tensor_batch.get("teacher_negative_multi_modal_inputs")
        )
        ...
        if has_negative_teacher_multi_modal_inputs:
            non_tensor_select_keys.append("teacher_negative_multi_modal_inputs")
        if self.use_prefix_grouper and "uid" in data.non_tensor_batch.keys():
            non_tensor_select_keys.append("uid")
        if token_trace_enabled:
            # Keep diagnostic metadata through DataProto.select/split and dynamic
            # micro-batching.  These fields are non-tensor arrays and are not
            # consumed by the model, but are required to interpret token traces.
            for key in (
                "uid",
                "ve_diag_index",
                "ve_diag_selected",
                ...
                "ve_task_group",
                "ve_evidence",
                "ve_verified_evidence",
                "ve_privileged_teacher_correct",
            ):
                ...
                for micro_batch in micro_batches:
                    ...
                    if self_distillation_enabled:
                        ...
                        with torch.no_grad():
                            ...
                            negative_teacher_outputs = None
                            if "teacher_negative_input_ids" in model_inputs:
                                negative_teacher_inputs = {
                                    "responses": model_inputs["responses"],
                                    "input_ids": model_inputs["teacher_negative_input_ids"],
                                    "attention_mask": model_inputs["teacher_negative_attention_mask"],
                                    "position_ids": model_inputs["teacher_negative_position_ids"],
                                    "response_start_idx": model_inputs["teacher_negative_response_start_idx"],
                                }
                                if "teacher_negative_multi_modal_inputs" in model_inputs:
                                    negative_teacher_inputs["multi_modal_inputs"] = model_inputs[
                                        "teacher_negative_multi_modal_inputs"
                                    ]
                        ...
                        if negative_teacher_outputs is not None:
                            privilege_logprob_gain = (
                                teacher_log_prob - negative_teacher_outputs["log_probs"]
                            ).detach()
                            privilege_margin = privilege_logprob_gain - float(
                                self_distillation_cfg.get("privilege_min_gain", 0.0)
                            )
                            privilege_gain = privilege_margin.clamp_min(0.0)
                            privilege_temperature = max(
                                float(self_distillation_cfg.get("privilege_temperature", 0.25)), 1e-4
                            )
                            privilege_beta = float(self_distillation_cfg.get("privilege_beta", 1.0))
                            privilege_positive = privilege_margin > 0.0
                            privilege_token_weight = 1.0 + privilege_beta * privilege_positive.to(
                                privilege_gain.dtype
                            ) * torch.sigmoid(privilege_gain / privilege_temperature)
                            micro_batch_metrics.update(
                                {
                                    "self_distillation/privilege_logprob_gain_mean": privilege_logprob_gain.float().mean().item(),
                                    "self_distillation/privilege_positive_fraction": (
                                        (
                                            privilege_logprob_gain
                                            > float(self_distillation_cfg.get("privilege_min_gain", 0.0))
                                        )
                                        .float()
                                        .mean()
                                        .item()
                            ...
                            micro_batch_metrics.update(
                                {
                                    "diagnostics/privilege_gain_valid_mean": 0.0,
                                    "diagnostics/privilege_positive_token_fraction": 0.0,
                                    "diagnostics/privilege_token_weight_valid_mean": 1.0,
                                    "diagnostics/response_valid_token_count": 0.0,
                                    "diagnostics/privilege_positive_sample_fraction": 0.0,
                                    "diagnostics/support_token_fraction": 0.0,
                                    "diagnostics/support_token_gap_mean": 0.0,
                                    "diagnostics/support_token_positive_fraction": 0.0,
                                    "diagnostics/student_topk_mass": 0.0,
                                    "diagnostics/privileged_teacher_mass_on_student_topk": 0.0,
                                    "diagnostics/privilege_topk_mass_gap": 0.0,
                                    "diagnostics/privileged_sample_fraction": 0.0,
                                    "diagnostics/native_control_sample_fraction": 1.0,
                                }
                            )
                            valid_token_mask = response_mask > 0
                            valid_gain = privilege_logprob_gain[valid_token_mask]
                            if valid_gain.numel() > 0:
                                micro_batch_metrics.update(
                                    {
                                        "diagnostics/privilege_gain_valid_mean": valid_gain.float().mean().item(),
                                        "diagnostics/privilege_positive_token_fraction": (
                                            valid_gain
                                            > float(self_distillation_cfg.get("privilege_min_gain", 0.0))
                                        )
                                        .float()
                                        .mean()
                                        .item(),
                                        "diagnostics/privilege_token_weight_valid_mean": privilege_token_weight[
                                            valid_token_mask
                                        ]
                                        .float()
                                        .mean()
                                        .item(),
                                        "diagnostics/response_valid_token_count": valid_token_mask.float()
                                        .sum(dim=1)
                                        .mean()
                                        .item(),
                                    }
                                )
                            row_has_positive_gap = (
                                (
                                    (privilege_logprob_gain > float(self_distillation_cfg.get("privilege_min_gain", 0.0)))
                                    & valid_token_mask
                                )
                                .any(dim=1)
                                .float()
                            )
                            micro_batch_metrics["diagnostics/privilege_positive_sample_fraction"] = (
                                row_has_positive_gap.mean().item()
                            )

                            support_token_ids = list(self_distillation_cfg.get("support_token_ids", []) or [])
                            if support_token_ids:
                                support_token_mask = torch.zeros_like(valid_token_mask, dtype=torch.bool)
                                for token_id in support_token_ids:
                                    support_token_mask |= model_inputs["responses"] == int(token_id)
                                support_token_mask &= valid_token_mask
                                support_count = support_token_mask.sum().item()
                                micro_batch_metrics["diagnostics/support_token_fraction"] = (
                                    support_token_mask.float().sum().item()
                                    / valid_token_mask.float().sum().clamp_min(1.0).item()
                                )
                                if support_count > 0:
                                    support_gain = privilege_logprob_gain[support_token_mask]
                                    micro_batch_metrics.update(
                                        {
                                            "diagnostics/support_token_gap_mean": support_gain.float().mean().item(),
                                            "diagnostics/support_token_positive_fraction": (
                                                support_gain
                                                > float(self_distillation_cfg.get("privilege_min_gain", 0.0))
                                            )
                                            .float()
                            ...
                            if student_topk_logps is not None and teacher_topk_logps is not None:
                                student_topk_mass = torch.exp(student_topk_logps).sum(dim=-1)
                                privileged_topk_mass = torch.exp(teacher_topk_logps).sum(dim=-1)
                                micro_batch_metrics.update(
                                    {
                                        "diagnostics/student_topk_mass": student_topk_mass[valid_token_mask]
                                        .float()
                                        .mean()
                                        .item(),
                                        "diagnostics/privileged_teacher_mass_on_student_topk": privileged_topk_mass[
                                            valid_token_mask
                                        ]
                                        .float()
                                        .mean()
                                        .item(),
                                        "diagnostics/privilege_topk_mass_gap": (
                                            privileged_topk_mass - student_topk_mass
                                        )[valid_token_mask]
                                        .float()
                                        .mean()
                                        .item(),
                            ...
                            role_values = model_inputs.get("ve_data_role")
                            if role_values is not None:
                                privileged_rows = torch.tensor(
                                    [
                                        str(role) in {"verified_privilege", "privileged", "privilege"}
                                ...
                                micro_batch_metrics.update(
                                    {
                                        "diagnostics/privileged_sample_fraction": privileged_rows.mean().item(),
                                        "diagnostics/native_control_sample_fraction": (1.0 - privileged_rows)
                                        .mean()
                                        .item(),
                                    }
                                )
                            if token_trace_enabled and "ve_diag_selected" in model_inputs:
                                selected_rows = [
                                    row_idx
                                    for row_idx, selected in enumerate(model_inputs["ve_diag_selected"])
                                    if bool(selected)
                                ]

                                def _trace_metadata_value(key, local_idx, default):
                                    values = model_inputs.get(key)
                                    try:
                                        return values[local_idx] if values is not None else default
                                    except (IndexError, KeyError, TypeError):
                                        pass
                                    try:
                                        global_idx = int(model_inputs["ve_diag_index"][local_idx])
                                        values = data.non_tensor_batch.get(key)
                                        return values[global_idx] if values is not None else default
                                    except (IndexError, KeyError, TypeError, ValueError):
                                        return default

                                for row_idx in selected_rows:
                                    valid = response_mask[row_idx] > 0
                                    sample_index = int(model_inputs["ve_diag_index"][row_idx])
                                    trace_mask = model_inputs.get("self_distillation_mask")
                                    trace_mask_value = (
                                        float(trace_mask[row_idx].detach().float().cpu().item())
                                        if trace_mask is not None
                                        else 0.0
                                    )
                                    token_trace_records.append(
                                        {
                                            "global_step": int(data.meta_info.get("global_steps", -1)),
                                            "sample_index": sample_index,
                                            "uid": str(_trace_metadata_value("uid", row_idx, "")),
                                            "role": str(_trace_metadata_value("ve_data_role", row_idx, "unknown")),
                                            "task_group": str(_trace_metadata_value("ve_task_group", row_idx, "unknown")),
                                            "verified_evidence": bool(
                                                _trace_metadata_value("ve_verified_evidence", row_idx, False)
                                            ),
                                            "privileged_teacher_correct": bool(
                                                _trace_metadata_value("ve_privileged_teacher_correct", row_idx, False)
                                            ),
                                            "self_distillation_mask": trace_mask_value,
                                            "has_privileged_teacher": trace_mask_value > 0.5,
                                            "evidence": str(_trace_metadata_value("ve_evidence", row_idx, "")),
                                            "response_token_ids": model_inputs["responses"][row_idx][valid]
                                            .detach()
                                            .cpu()
                                            .tolist(),
                                            "native_log_probs": negative_teacher_outputs["log_probs"][row_idx][valid]
                                            .detach()
                                            .float()
                                            .cpu()
                                            .tolist(),
                                            "privileged_log_probs": teacher_log_prob[row_idx][valid]
                                            .detach()
                                            .float()
                                            .cpu()
                                            .tolist(),
                                            "privilege_gap": privilege_logprob_gain[row_idx][valid]
                                            .detach()
                                            .float()
                                            .cpu()
                                            .tolist(),
                                            "privilege_token_weight": privilege_token_weight[row_idx][valid]
                                            .detach()
                                            .float()
                                            .cpu()
                                            .tolist(),
                        ...
                        vopd_loss, vopd_metrics = compute_self_distillation_loss(
                            ...
                            privilege_token_weight=privilege_token_weight,
        ...
        if token_trace_enabled and token_trace_records:
            self._dump_ve_opd_token_traces(
                meta_info=data.meta_info,
                self_distillation_cfg=self_distillation_cfg,
                records=token_trace_records,
            )
        if did_update:
