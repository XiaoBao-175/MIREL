...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        ...
        teacher_negative_image_key (Optional[str]): Native-view image column used to contrast the privileged teacher.
        privilege_beta (float): Extra distillation weight for tokens improved by the privileged view.
        privilege_temperature (float): Temperature for the token-level privilege gate.
        privilege_min_gain (float): Minimum teacher log-probability gain before a token is emphasized.
    ...
    teacher_negative_image_key: Optional[str] = None
    privilege_beta: float = 1.0
    privilege_temperature: float = 0.25
    privilege_min_gain: float = 0.0
    ...
    token_trace_dump_dir: Optional[str] = None
    token_trace_every_n_steps: int = 1
    token_trace_samples_per_role: int = 2
    token_trace_seed: int = 20260911
    support_token_ids: list[int] = field(default_factory=list)
    ...
    def __post_init__(self):
        ...
        if self.token_trace_every_n_steps <= 0:
            raise ValueError("self_distillation.token_trace_every_n_steps must be positive")
        if self.token_trace_samples_per_role < 0:
            raise ValueError("self_distillation.token_trace_samples_per_role must be non-negative")
