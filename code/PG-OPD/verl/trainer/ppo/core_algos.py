...
def compute_self_distillation_loss(
    ...
    privilege_token_weight: Optional[torch.Tensor] = None,
    ...
    # Give extra weight only to response tokens whose probability improves
    # under the evidence-privileged teacher. Native rows have identical
    # privileged/native prompts, so their gain is zero and retain unit weight.
    if privilege_token_weight is not None:
        privilege_token_weight = privilege_token_weight.to(loss_mask.dtype).clamp(min=0.0)
        weighted_loss_mask = loss_mask * privilege_token_weight
    else:
    ...
    weighted_valid_token_count = weighted_loss_mask.sum().clamp(min=1.0)
    if batch_num_tokens is None:
    ...
    metrics["self_distillation/weighted_jsd_token_mean"] = (
        verl_F.masked_sum(weighted_per_token_loss, weighted_loss_mask) / weighted_valid_token_count
    ).detach().item()
    ...
    if privilege_token_weight is None:
        metrics["self_distillation/privilege_token_weight.mean()"] = 1.0
    else:
        metrics["self_distillation/privilege_token_weight.mean()"] = privilege_token_weight.float().mean().detach().item()
        metrics["self_distillation/privilege_positive_fraction"] = (
            (privilege_token_weight > 1.0).float().mean().detach().item()
        )
    metrics["self_distillation/num_distill_tokens"] = weighted_loss_mask.sum().detach().item()
