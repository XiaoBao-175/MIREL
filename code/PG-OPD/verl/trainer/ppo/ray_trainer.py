...
class RayPPOTrainer:
    ...
    def _maybe_build_self_distillation_batch(
        ...
        if use_teacher_always_on_inputs:
            ...
            teacher_negative_image_key = self_distillation_cfg.get("teacher_negative_image_key", None)
            ...
            if teacher_negative_image_key is not None and teacher_negative_image_key not in batch.non_tensor_batch:
                raise KeyError(
                    f"Negative teacher image key `{teacher_negative_image_key}` not found in batch.non_tensor_batch"
                )
            ...
            teacher_response_start_idx_list = []
            teacher_multi_modal_inputs_list = []
            negative_teacher_input_ids_list = []
            negative_teacher_attention_mask_list = []
            negative_teacher_position_ids_list = []
            negative_teacher_response_start_idx_list = []
            negative_teacher_multi_modal_inputs_list = []
            teacher_present_mask_list = []
            ...
            for i in range(batch_size):
                ...
                if teacher_negative_image_key is not None:
                    negative_images = batch.non_tensor_batch[teacher_negative_image_key][i]
                    if isinstance(negative_images, np.ndarray):
                        negative_images = negative_images.tolist()
                    elif negative_images is None:
                        negative_images = []
                    else:
                        negative_images = list(negative_images)
                    if not self._teacher_images_available(negative_images):
                        raise ValueError(
                            f"Negative teacher image key `{teacher_negative_image_key}` is empty for sample {i}."
                        )
                    negative_teacher_messages = self._prepare_teacher_messages(
                        list(batch.non_tensor_batch["raw_prompt"][i]),
                        negative_images,
                        teacher_prompt_messages=(
                            list(batch.non_tensor_batch["teacher_negative_prompt"][i])
                            if "teacher_negative_prompt" in batch.non_tensor_batch
                            else None
                        ),
                    )
                    (
                    ...
                    negative_teacher_input_ids_list.append(negative_teacher_input_ids)
                    negative_teacher_attention_mask_list.append(negative_teacher_attention_mask)
                    negative_teacher_position_ids_list.append(negative_teacher_position_ids)
                    negative_teacher_response_start_idx_list.append(negative_teacher_response_start_idx)
                    negative_teacher_multi_modal_inputs_list.append(negative_teacher_multi_modal_inputs)
            ...
            if teacher_negative_image_key is not None:
                negative_teacher_input_ids = torch.nn.utils.rnn.pad_sequence(
                    negative_teacher_input_ids_list,
                    batch_first=True,
                    padding_value=self.tokenizer.pad_token_id or 0,
                ).to(device)
                negative_teacher_attention_mask = torch.nn.utils.rnn.pad_sequence(
                    negative_teacher_attention_mask_list,
                    batch_first=True,
                    padding_value=0,
                ).to(device)
                max_negative_teacher_len = negative_teacher_input_ids.shape[1]
                if negative_teacher_position_ids_list[0].dim() == 1:
                    negative_teacher_position_ids = torch.zeros(
                        (batch_size, max_negative_teacher_len),
                        dtype=negative_teacher_position_ids_list[0].dtype,
                        device=device,
                    )
                    for i, position_ids in enumerate(negative_teacher_position_ids_list):
                        negative_teacher_position_ids[i, : position_ids.shape[-1]] = position_ids.to(device)
                else:
                    negative_rope_dims = negative_teacher_position_ids_list[0].shape[0]
                    negative_teacher_position_ids = torch.zeros(
                        (batch_size, negative_rope_dims, max_negative_teacher_len),
                        dtype=negative_teacher_position_ids_list[0].dtype,
                        device=device,
                    )
                    for i, position_ids in enumerate(negative_teacher_position_ids_list):
                        negative_teacher_position_ids[i, :, : position_ids.shape[-1]] = position_ids.to(device)
                teacher_tensors.update(
                    {
                        "teacher_negative_input_ids": negative_teacher_input_ids,
                        "teacher_negative_attention_mask": negative_teacher_attention_mask,
                        "teacher_negative_position_ids": negative_teacher_position_ids,
                        "teacher_negative_response_start_idx": torch.stack(
                            negative_teacher_response_start_idx_list
                        ).to(device),
                    }
                )
                teacher_non_tensors["teacher_negative_multi_modal_inputs"] = negative_teacher_multi_modal_inputs_list
            ...
            teacher_tensors["self_distillation_mask"] = teacher_present_mask
            return DataProto.from_dict(
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        ...
        # Async rollout returns generated tensors but does not reliably carry
        # arbitrary non-tensor columns back into the training batch.  Preserve
        # VE-OPD diagnostics here so actor-side token traces retain provenance.
        reward_model_keys.update(
            {
                "ve_data_role",
                "ve_task_group",
                "ve_evidence",
        ...
        teacher_negative_image_key = self.config.actor_rollout_ref.actor.get("self_distillation", {}).get(
            "teacher_negative_image_key", None
        )
        if teacher_negative_image_key and teacher_negative_image_key in batch.non_tensor_batch:
            reward_model_keys.add(teacher_negative_image_key)
